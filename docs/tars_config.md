...
Currently, the service service obtains application-level configuration information by using the addAppConfig interface provided in the framework, which is responsible for pulling the corresponding application configuration file from the configServer service to the local directory.
```
...
1.First, the service name, the configuration file name, the set group information, and the configuration information level are used as search conditions, acquiring and the configuration information and index id whose configuration information level is "service level".

2.Secondly, the relation of obtaining corresponding configuration information index id (reference_id) and condig_id which are referenced by index id in (1) from data table (t_config_referencses) is as follows:
...
In the service tree on the left, select the service service which need to be maintained. Click the “Service Configuration” page in the management interface on the left to add, edit, and delete the service configuration. As shown below:
